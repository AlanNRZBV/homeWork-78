...
categoriesRouter.get('/', async (req, res, next) => {
  try {
    const [results] = await mysqlDb.getConnection().query('SELECT id, name FROM categories');
    res.send(results);
  } catch (e) {
    next(e);
  }
...
categoriesRouter.get('/:id', async (req, res, next) => {
  try {
    const [results] = (await mysqlDb
      .getConnection()
      .query('SELECT * FROM categories WHERE id = ?', [req.params.id])) as RowDataPacket[];
...
categoriesRouter.delete('/:id', async (req, res, next) => {
  try {
    const [isExist] = (await mysqlDb
      .getConnection()
      .query('SELECT id FROM categories WHERE id = ?', [req.params.id])) as RowDataPacket[];
